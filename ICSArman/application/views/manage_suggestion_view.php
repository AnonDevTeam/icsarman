...
<div class="tab-pane fade" id="other_suggestion">
	...
	<table class="table table-bordered">
		...
		<form enctype="multipart/form-data" action='/ICSArman/index.php/add_material/callAddMaterial' method="post">
			<tr>
				<td><input type = "text" readonly="readonly" name = "auxil_id"  value = '<?php echo $other['auxil_id'];?>' /></td>
				<td><?php echo $other['id'];?></td>
				<td><input type = "text" name="title" value='<?php echo $other['title'];?>'/></td>
				<td><input type = "text" name="author" value='<?php echo $other['author'];?>'/></td>
				<td><input type = "number" name="year" value='<?php echo $other['date_published'];?>'/></td>
