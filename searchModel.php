<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class SearchModel extends CI_Model{
		...
		function checkReserveMaterial($user_id, $mat_id){
			$res = $this->db->query("SELECT id, material_id from reserved_material where material_id = $mat_id AND id = $user_id");
			return $res->result();
		...
		function searchAll($input){	//search if the input is title | author | publisher | adviser | year
			...
			$check['material'] = $this->db->query("SELECT material_id from material where title LIKE '$input%' OR author LIKE '$input%' OR year = $x")->result();
